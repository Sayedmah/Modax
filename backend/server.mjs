...
const builtinProviders={
 ...
 openrouter:{id:'openrouter',label:'OpenRouter',kind:'openai-compatible',baseUrl:'https://openrouter.ai/api/v1',keyEnv:'OPENROUTER_API_KEY',modelsPath:'/models',chatPath:'/chat/completions',extraHeaders:()=>({...process.env.MODY_PUBLIC_URL?{'HTTP-Referer':process.env.MODY_PUBLIC_URL}:{},'X-Title':'Modax AI'})}
};
...
async function handle(req,res){
 ...
 if(req.method==='POST'&&url.pathname==='/v1/multi-ai'){await requireUser(req);const body=await readJson(req);if(!Array.isArray(body.targets)||body.targets.length<2||body.targets.length>8)return sendJson(res,400,{error:'targets must contain 2-8 {provider, model} entries'},origin);const messages=normalizeMessages(body.messages,body.prompt),results=await Promise.all(body.targets.map(async target=>{const started=Date.now();try{const p=getProvider(target.provider),out=p.id==='google'&&target.model==='auto'?await autoGeminiChat({...body,messages,model:'auto'}):await chat(p,{...body,model:target.model,messages});return{ok:true,provider:p.id,model:out.model||target.model,latency_ms:Date.now()-started,...out};}catch(err){return{ok:false,provider:target.provider,model:target.model,latency_ms:Date.now()-started,error:err.message};}}));return sendJson(res,200,{independent:true,results},origin);}
 const match=req.method==='POST'&&url.pathname.match(/^\/v1\/game-studio\/(create|repair|build)$/);if(match){await requireUser(req);const body=await readJson(req),result=await routeGodotApi(match[1],body,{ai:gameStudioAI,queueBuild:queueGodotBuild});return sendJson(res,result.status,result.body,origin);}
